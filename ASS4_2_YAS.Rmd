...
setwd("~/Library/Mobile Documents/com~apple~CloudDocs/4. Aarhus Universitet/3. Semester/1. Experimental Methods lll/R FOLDER/HR/HR")

locpath <- setwd("~/Library/Mobile Documents/com~apple~CloudDocs/4. Aarhus Universitet/3. Semester/1. Experimental Methods lll/R FOLDER/HR/HR")
```
...
```{r, collecting files}
##list of file names
files <- list.files(path = locpath, pattern="Study*", full.names = F) 
```
...
```{r vanish}
# OBS OBS OBS TAKES A LONG TIME TO RUN
...
#Tryout: loading
all_preprocessed = read.csv("preprocessed_from_vanish.csv")
#works
```
